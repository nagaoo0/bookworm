...
- `books` holds canonical metadata — one row per unique title. `google_id` / `open_library_id` / `apple_id` are used for upsert on re-add; manual books have all three `= NULL` and always insert a new row.
